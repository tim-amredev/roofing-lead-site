document.addEventListener('DOMContentLoaded', function() {
  ...
  const pricing = {
    materials: {
      asphalt: { low: 4.50, high: 7.50 },
      premium: { low: 6.50, high: 11.00 },
      metal: { low: 9.00, high: 17.00 },
      wood: { low: 8.00, high: 15.00 },
      tile: { low: 13.00, high: 27.00 },
      slate: { low: 20.00, high: 45.00 }
    },
    pitchMultipliers: {
      flat: 1.0,
      low: 1.1,
    ...
    additions: {
      removal: { low: 1.50, high: 2.50 },  // per sq ft
      underlayment: { flat: 500, high: 1000 },  // flat fee
      ventilation: { flat: 600, high: 1200 },   // flat fee
      gutters: { low: 4.00, high: 8.00 },       // per sq ft
      insulation: { low: 2.00, high: 4.00 }     // per sq ft
    }
  };
  ...
  function calculateEstimate() {
    ...
    // Calculate base cost
    let lowBase = area * pricing.materials[material].low * pricing.pitchMultipliers[pitch] * pricing.regionMultipliers[region];
    let highBase = area * pricing.materials[material].high * pricing.pitchMultipliers[pitch] * pricing.regionMultipliers[region];
    ...
    const averageEstimate = Math.round((lowBase + highBase) / 2).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
    ...
    document.getElementById('result-material').textContent = document.querySelector(`label[for="material-${material}"] h3`).textContent;
    document.getElementById('result-pitch').textContent = document.querySelector(`#roof-pitch option[value="${pitch}"]`).textContent;
    document.getElementById('result-location').textContent = document.querySelector(`#location option[value="${region}"]`).textContent;
